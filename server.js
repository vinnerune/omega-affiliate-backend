...
// migrations: add columns that may not exist in an older DB (ignore if present)
try { db.exec("ALTER TABLE affiliates ADD COLUMN elite_license TEXT"); } catch (e) {}
try { db.exec("ALTER TABLE affiliates ADD COLUMN commission_rate REAL"); } catch (e) {}
...
// (avoids third-party-cookie blocking on cross-site setups).
...
app.use((req, res, next) => {                       // CORS — token auth (no cookies), echo origin
...
function buildDashboard(a) {
  ...
  return {
    ...
    commissionRate: (typeof a.commission_rate === "number" ? a.commission_rate : COMMISSION_RATE),
...
app.post("/api/admin/affiliates/:code/approve", admin, (req, res) => {
  ...
  const info = db.prepare("UPDATE affiliates SET status='approved', password_hash=?, commission_rate=COALESCE(commission_rate, ?) WHERE code=?")
    .run(bcrypt.hashSync(pw, 10), COMMISSION_RATE, req.params.code);
  const a = db.prepare("SELECT code,name,email,paypal FROM affiliates WHERE code=?").get(req.params.code);
  res.json({ ok: info.changes > 0, tempPassword: pw, affiliate: a, eliteDownload: ELITE_DOWNLOAD, link: `${SITE_ORIGIN}/?ref=${req.params.code}` });
});
// store the REAL Elite license (minted by the license webhook) on the affiliate
app.post("/api/admin/affiliates/:code/license", admin, (req, res) => {
  const lic = (req.body && req.body.license) || "";
  if (!lic) return res.status(400).json({ ok: false, error: "license required" });
  const info = db.prepare("UPDATE affiliates SET elite_license=? WHERE code=?").run(lic, req.params.code);
  res.json({ ok: info.changes > 0 });
});
// approved affiliates WITH full stats (for the admin table)
app.get("/api/admin/affiliates", admin, (req, res) => {
  const list = db.prepare("SELECT * FROM affiliates WHERE status='approved' ORDER BY created_at DESC").all();
  res.json(list.map((a) => Object.assign(buildDashboard(a), {
    email: a.email, paypal: a.paypal || a.email, createdAt: a.created_at
  })));
});
// dashboard totals for the admin KPI cards
app.get("/api/admin/stats", admin, (req, res) => {
  const pending = db.prepare("SELECT COUNT(*) c FROM affiliates WHERE status='pending'").get().c;
  const affiliates = db.prepare("SELECT COUNT(*) c FROM affiliates WHERE status='approved'").get().c;
  const sales = db.prepare("SELECT COUNT(*) c FROM purchases").get().c;
  const earned = db.prepare("SELECT COALESCE(SUM(commission),0) s FROM purchases").get().s;
  const paid = db.prepare("SELECT COALESCE(SUM(amount),0) s FROM payouts WHERE status='Paid'").get().s;
